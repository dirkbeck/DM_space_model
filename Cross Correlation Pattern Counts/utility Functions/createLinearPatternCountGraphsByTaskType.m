...
% binSize = 1;
function [] = createLinearPatternCountGraphsByTaskType(binSize,allFilesToLoad,sortedTableOfSkews,whichDatabaseYouWantToView)

switch strrep(lower(whichDatabaseYouWantToView),' ','')
    case "control"
        beginning = 1;
        ending = 1.5;
    case "stress"
    ...
    case "stress2"
        ...
        ending = 3.5;
    otherwise 
        warning('Did not enter a valid database. ')
        return
end
...
%these files in have their maps split by task type 
...
hasConcentrations = false;   %this boolean is used to tell the function whether the task types in the current map have concentrations attached to them or not
...
for currentFile =beginning:ending%length(allFilesToLoad)
    ...
    for longOrShort =1:length(bothPatternCountMapsTogether)
        ...
        %         display(tableOfPatternCountswithJust1TaskType)
        allTaskTypeAndConcentrations = split(tableOfPatternCounts.("Task Type and Concentration"),"Type",2);
        ...
        justTaskType = allTaskTypeAndConcentrations(:,2);
        %         disp(justConcentration)
        ...
        subtitle("Created By createLinearPatternCountGraphsByTaskType.m")
