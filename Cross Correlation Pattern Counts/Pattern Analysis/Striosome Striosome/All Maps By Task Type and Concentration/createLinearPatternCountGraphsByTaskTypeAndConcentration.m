...
%         disp(tableOfPatternCounts)
        ...
        tableOfPatternCountsWithPatternsInOwnColumn = table(tableOfPatternCounts.("Task Type and Concentration"),allPatternCounts(:,1),allPatternCounts(:,2),allPatternCounts(:,3),allPatternCounts(:,4),tableOfPatternCounts.("Skewness"),...
            'VariableNames',{'Task Type and Concentration','Excited Excited','Excited Inhibited','Inhibited Excited','Inhibited Inhibited','Skewness'});
        disp(tableOfPatternCountsWithPatternsInOwnColumn)
        theSaveNameAsMat = strcat(databases(currentDB)," ",longOrShortTitles(longOrShort)," Pattern Counts Table.mat");
        theSaveNameAsXLSX = strcat(databases(currentDB)," ",longOrShortTitles(longOrShort)," Pattern Counts Table.xlsx");
        save(theSaveNameAsMat,"tableOfPatternCountsWithPatternsInOwnColumn")
        writetable(tableOfPatternCountsWithPatternsInOwnColumn,theSaveNameAsXLSX)

        allTheTaskTypes = [" TR ","e CB "," Rev CB "," EQR "]; %a list of all possible task types in the database
        for i=1:length(allTheTaskTypes)
%         disp(allTheTaskTypes(i))
%         disp("__________________________________________________________________________")
        tableOfPatternCountswithJust1TaskType = tableOfPatternCounts(contains(tableOfPatternCounts.("Task Type and Concentration"),allTheTaskTypes(i)),:);
%         display(tableOfPatternCountswithJust1TaskType)
        x = tableOfPatternCountswithJust1TaskType.Skewness;
%         display(tableOfPatternCountswithJust1TaskType)
        allTaskTypeAndConcentrations = split(tableOfPatternCountswithJust1TaskType.("Task Type and Concentration")," ",2);
%         disp(allTaskTypeAndConcentrations)
        justConcentration = allTaskTypeAndConcentrations(:,end);
        justConcentration = strcat(justConcentration,"%");
%         disp(justConcentration)


        allys = tableOfPatternCountswithJust1TaskType.("Pattern Counts");
        allys(isnan(allys)) = 0;
%         display(x)
%         display(allys)
        ...
        text(x,eey,justConcentration)
        %" Rev CB "," EQR "

        if strcmpi(databases(currentDB),"Stress ") && strcmpi(allTheTaskTypes(i)," EQR ")
            close(gcf)
            continue
        elseif strcmpi(databases(currentDB),"Stress ") && strcmpi(allTheTaskTypes(i)," Rev CB ")
            close(gcf)
            continue
        elseif strcmpi(databases(currentDB),"Stress2") && strcmpi(allTheTaskTypes(i)," Rev CB ")
            close(gcf)
            continue
        elseif strcmpi(databases(currentDB),"Stress2") && strcmpi(allTheTaskTypes(i)," EQR ")
            close(gcf)
            continue
        end

        if ~exist("Pattern Counts as Line Graphs", 'dir')
...
%         disp(pwd)
        ...
        legend(patternNames,'Location','northwest')
        saveName = strcat(databases(currentDB)," ",longOrShortTitles(longOrShort)," Pattern Chart Task Type ", allTheTaskTypes(i), " Bin Size is ",string(binSize),".fig");
%         saveas(gcf,saveName)
%         close(gcf)
        cd(homeDir);
        end
