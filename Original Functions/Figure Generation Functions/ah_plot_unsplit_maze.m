function ah_plot_unsplit_maze(middle_plotting_bins, right_plotting_bins, left_plotting_bins, fig_marker, figure_params)
%AH_PLOT_UNSPLIT_MAZE takes in three sets of plotting bins and 11 paramters
%relevant to the figure being created and plots a maze figure with the
%middle not split. Inputs are:
...
%   - maze_arm_length - the length of the arms of the actual T-maze
...
%       color or the absolute low color value
%   - high_color - the percentile of the value that we set as the highest
%       color or the absolute high color
...
numBins = size(middle_plotting_bins,1);     %number of bins
...
if coloring_type
    lowColorValMiddle = low_color;
    lowColorValRight = low_color;
    lowColorValLeft = low_color;
    highColorValMiddle = high_color;
    ...
    tmp = sort(middle_plotting_bins);           %temporary variable; sorted middle bins
    lowColorValMiddle = tmp(idLowColor);        %low color value of middle bins
    highColorValMiddle = tmp(idHighColor);      %high color value of middle bins
...
%% Stem Bins
for i = 1:premaze_length*numBins + maze_height_num  %loop through bins in stem of figure
    if i <= premaze_length*numBins
        ...
        tmp = max(min(middle_plotting_bins(i),highColorValMiddle),lowColorValMiddle);   %temporary variable; shifting value if above max or below min
        color = round(abs(((tmp-lowColorValMiddle)/(highColorValMiddle-lowColorValMiddle))^(exp_param))*200.999999999+.5); color = max(color,1); color = min(color,201);    %row of color appropriate for value
        rectangle('Position', [-maze_width/2, (i-1)*rectangle_width, maze_width, rectangle_width], 'FaceColor', cmap(color,:), 'EdgeColor', 'none') %draw bin
    else
        rectangle_width = maze_height/maze_height_num;                                  %width of rectangle we're plotting
        j = i - premaze_length*numBins;                                                 %helper variable
        tmp = max(min(middle_plotting_bins(i),highColorValMiddle),lowColorValMiddle);   %temporary variable; shifting value if above max or below min
        color = round(abs(((tmp-lowColorValMiddle)/(highColorValMiddle-lowColorValMiddle))^(exp_param))*200.999999999+.5); color = max(color,1); color = min(color,201);    %row of color appropriate for value
        rectangle('Position', [-maze_width/2,(j-1)*rectangle_width+extended_height-maze_height,maze_width,rectangle_width], 'FaceColor', cmap(color,:), 'EdgeColor','none') %draw bin
    end
end
%% Arm Bins
for i = 1:maze_length_num+postmaze_length*numBins
    if i+maze_height_num+maze_middle_num <= maze_length*numBins
        ...
        rectangle('Position', [maze_width/2+rectangle_width*(i-1), extended_height, rectangle_width, maze_width], 'FaceColor', cmap(rightColor,:), 'EdgeColor', 'none')     %draw bin
        rectangle('Position', [-maze_width/2-rectangle_width*i, extended_height, rectangle_width, maze_width], 'FaceColor', cmap(leftcolor,:), 'EdgeColor', 'none')         %draw bin
    else
        ...
        j = i - maze_length_num;
        rectangle('Position', [maze_width/2+maze_arm_length+rectangle_width*(j-1), extended_height, rectangle_width, maze_width], 'FaceColor', cmap(rightColor,:), 'EdgeColor', 'none') %draw bin
        rectangle('Position', [-maze_width/2-maze_arm_length-rectangle_width*j, extended_height, rectangle_width, maze_width], 'FaceColor', cmap(leftcolor,:), 'EdgeColor', 'none')     %draw bin
    end
end
%% Center Bins
theta = pi/(2*maze_middle_num);
for i = 1:maze_middle_num
    ...
    if tan(i*theta) < 2
        patch([maze_width/2; 0; 0], [extended_height; extended_height+tan(i*theta)*maze_width/2; extended_height+tan((i-1)*theta)*maze_width/2], cmap(rightColor,:), 'EdgeColor', 'none')   %draw bin
        patch([-maze_width/2; 0; 0], [extended_height; extended_height+tan(i*theta)*maze_width/2; extended_height+tan((i-1)*theta)*maze_width/2], cmap(leftcolor,:), 'EdgeColor', 'none')   %draw bin
    elseif tan((i-1)*theta) < 2
        patch([maze_width/2; 0; 0; maze_width/2-maze_width*cot(i*theta)], [extended_height; extended_height+tan((i-1)*theta)*maze_width/2; extended_height+maze_width; extended_height+maze_width], cmap(rightColor,:), 'EdgeColor', 'none')    %draw bin
        patch([-maze_width/2; 0; 0; -maze_width/2+maze_width*cot(i*theta)], [extended_height; extended_height+tan((i-1)*theta)*maze_width/2; extended_height+maze_width; extended_height+maze_width], cmap(leftcolor,:), 'EdgeColor', 'none')   %draw bin
    else
        patch([maze_width/2; maze_width/2-maze_width*cot(i*theta); maze_width/2-maze_width*cot((i-1)*theta)], [extended_height; extended_height+maze_width; extended_height+maze_width], cmap(rightColor,:), 'EdgeColor', 'none')   %draw bin
        patch([-maze_width/2; -maze_width/2+maze_width*cot(i*theta); -maze_width/2+maze_width*cot((i-1)*theta)], [extended_height; extended_height+maze_width; extended_height+maze_width], cmap(leftcolor,:), 'EdgeColor', 'none') %draw bin
    end
end
%% Legend
cmap_base = 0;
cmap_top = extended_height - maze_width;
cmap_left = extended_length - maze_width/2;
cmap_right = extended_length + maze_width/2;
...
%% Maze Outline
line([-maze_width/2 maze_width/2], [0 0], 'LineWidth', 3, 'Color', 'Black', 'LineStyle', ':')
line([-maze_width/2 maze_width/2], [extended_height-maze_height extended_height-maze_height], 'LineWidth', 3, 'Color', 'Black', 'LineStyle', '-')
line([-maze_width/2 -maze_width/2], [0 extended_height-maze_height], 'LineWidth', 3, 'Color', 'Black', 'LineStyle', ':')
line([maze_width/2 maze_width/2], [0 extended_height-maze_height], 'LineWidth', 3, 'Color', 'Black', 'LineStyle', ':')
line([-maze_width/2 -maze_width/2], [extended_height-maze_height extended_height], 'LineWidth', 3, 'Color', 'Black', 'LineStyle', '-')
line([maze_width/2 maze_width/2], [extended_height-maze_height extended_height], 'LineWidth', 3, 'Color', 'Black', 'LineStyle', '-')
line([-maze_width/2 -maze_width/2-maze_arm_length], [extended_height extended_height], 'LineWidth', 3, 'Color', 'Black', 'LineStyle', '-')
line([maze_width/2 maze_width/2+maze_arm_length], [extended_height extended_height], 'LineWidth', 3, 'Color', 'Black', 'LineStyle', '-')
line([-maze_width/2-maze_arm_length maze_width/2+maze_arm_length], [extended_height+maze_width extended_height+maze_width],  'LineWidth', 3, 'Color', 'Black', 'LineStyle', '-')
line([-maze_width/2-maze_arm_length -maze_width/2-maze_arm_length], [extended_height+maze_width extended_height],  'LineWidth', 3, 'Color', 'Black', 'LineStyle', '-')
line([maze_width/2+maze_arm_length maze_width/2+maze_arm_length], [extended_height+maze_width extended_height],  'LineWidth', 3, 'Color', 'Black', 'LineStyle', '-')
line([-maze_width/2-maze_arm_length -maze_width/2-extended_length], [extended_height extended_height],  'LineWidth', 3, 'Color', 'Black', 'LineStyle', ':')
line([-maze_width/2-maze_arm_length -maze_width/2-extended_length], [extended_height+maze_width extended_height+maze_width],  'LineWidth', 3, 'Color', 'Black', 'LineStyle', ':')
line([-maze_width/2-extended_length -maze_width/2-extended_length], [extended_height extended_height+maze_width],  'LineWidth', 3, 'Color', 'Black', 'LineStyle', ':')
line([maze_width/2+maze_arm_length maze_width/2+extended_length], [extended_height extended_height],  'LineWidth', 3, 'Color', 'Black', 'LineStyle', ':')
line([maze_width/2+maze_arm_length maze_width/2+extended_length], [extended_height+maze_width extended_height+maze_width],  'LineWidth', 3, 'Color', 'Black', 'LineStyle', ':')
line([maze_width/2+extended_length maze_width/2+extended_length], [extended_height extended_height+maze_width],  'LineWidth', 3, 'Color', 'Black', 'LineStyle', ':')

xlim([-extended_length-1.5*maze_width, extended_length+1.5*maze_width])
